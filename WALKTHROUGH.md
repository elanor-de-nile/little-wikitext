...
* [`bin`](./bin): Contains `markup2html`, a simple CLI tool to exercise the
  code.
* [`build`](./build): Contains tools used to generate documentation; it can be ignored.
...
* [`LittleWikitext`](src/LittleWikitext.php): An entry point class

### Markup grammar ([Parser](src/Parser) folder)
...
### Abstract Syntax Tree ([AST](src/AST) folder)
...
### Visitor pattern ([Visitors](src/Visitors) folder)
...
```sh
$ php bin/markup2html sample.markup
````
...
the `full-test` target.  Many code style issues can be automatically
fixed with the `composer fix` target.
